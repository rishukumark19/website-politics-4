        ...
        <div className="max-w-[1400px] mx-auto relative z-10 text-center">
          <Reveal>
            <div className="flex items-center justify-center gap-4 mb-8">
              ...
              <span className="text-[11px] font-black text-[#FF9933] uppercase tracking-[0.5em]">{t('gallery.tag')}</span>
              <div className="h-px w-12 bg-[#FF9933]" />
            </div>
          </Reveal>
          <Reveal delay={100}>
            <h1 className="text-5xl md:text-8xl font-headline font-black text-white max-w-5xl mx-auto leading-[1.1] tracking-tight uppercase">
              {t('gallery.title1')} <br />
              <span className="text-[#FF9933]">{t('gallery.title2')}</span>
            </h1>
          </Reveal>
          <Reveal delay={200}>
          ...
          {photoPostUrls.map((url, index) => (
            <Reveal key={index} delay={Math.min(index * 100, 400)}>
              <div className="group space-y-8">
                <div className="bg-[#f8f9fa] rounded-[2.5rem] overflow-hidden editorial-shadow p-6 transition-all duration-700 hover:-translate-y-4 hover:shadow-2xl border border-transparent hover:border-[#FF9933]/10">
                ...
                <div className="flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                  <div className="h-px flex-grow bg-[#FF9933]/20" />
                  <span className="text-[10px] font-black uppercase tracking-[0.4em] text-[#FF9933]">Archival Media</span>
                  <div className="h-px flex-grow bg-[#FF9933]/20" />
                </div>
              </div>
            </Reveal>
